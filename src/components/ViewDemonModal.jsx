...
export async function loader({request, params}) {
    ...
    const response = await fetch('https://gd-demon-tracker-be-production.up.railway.app:3001/demonlist/' + username + '/' + levelID, {
      });
...
export async function action({params}) {
    const levelID = params.levelID;
    const token = getAuthToken();
    const response = await fetch('https://gd-demon-tracker-be-production.up.railway.app:3001/demonlist/' + levelID, {
        method: 'DELETE',
            headers: {
